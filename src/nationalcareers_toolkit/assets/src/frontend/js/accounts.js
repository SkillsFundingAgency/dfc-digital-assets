$(document).ready(function () {
    ...
    var button = $("#edit-your-detailsGovukButtonSave");
    ...
    function toggleItemErrorClass(item, hasError) {
        ...
        if (hasError && !errorClass) {
            ...
            toggleFormGroupErrorClass(item, hasError);
    ...
    function toggleFormGroupErrorClass(item, hasErrors) {
        var formGroup = item.parentNode;
        var errorClass = $(formGroup).hasClass(formErrorClass);

        if (hasErrors && !errorClass) {
            $(formGroup).addClass(formErrorClass);
        } else if (!hasErrors && errorClass) {
            $(formGroup).removeClass(formErrorClass);
        }
    }
    ...
    function focusout(input) {
        ...
        var validationMsg = $(parent).children("span.govuk-error-message.field-validation-error");

        updateErrorDetails(input, $(validationMsg).text(), $(validationMsg).length > 0);
    }
    ...
    $(form).on('submit', validateInputs);
});

/**
 
 
 Legacy SHC Validation Code
 ...
 *
 *
 * End Legacy SHC Validation Code
 *
 * ****
 */